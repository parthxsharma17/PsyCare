...
const allowRandomFallback = isDevelopment && process.env.ALLOW_RANDOM_ML_FALLBACK === 'true';
...
exports.analyzeMood = async (req, res) => {
  try {
    ...
    try {
      ...
      if (mlError.response && isDevelopment) {
      ...
      if (!allowRandomFallback) {
        return res.status(503).json({
          success: false,
          message: 'Mood analysis service is temporarily unavailable. Please try again later.',
          error: isDevelopment ? mlError.message : undefined
        });
      }

      // Development fallback only (explicitly enabled).
      try {
        ...
        const mood = await Mood.create({
          ...
          notes: 'Generated by development fallback (ML service unavailable)',
          capturedVia: 'ai'
        });
        ...
        return res.status(200).json({
          ...
          note: 'ML service unavailable, development fallback enabled'
        });
      } catch (fallbackError) {
        ...
        return res.status(500).json({
          ...
          error: isDevelopment ? fallbackError.message : undefined
        });
      }
    }
  } catch (error) {
